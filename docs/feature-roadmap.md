...
- Live write-validation harness with plan-only bundle capture by default and explicit execute-mode evidence capture for the current reversible controls, plus fan preset, restore-to-auto, and EnvyControl **GPU mode dry-run plan** capture when the probe exposes a switchable mode (no GPU or fan execution from the harness).
...
- Continue the GTK Fans scratchpad toward a fuller v0.2 fan editor (optional extra UX around the local sysfs preview, export snippets, etc.) after the planning surface is validated on fixtures and hardware; keep `ApplyFanPreset` / `RestoreAutoFan` execution gated until the live write-validation checklist is satisfied.
