...
- Continue the GTK Fans scratchpad toward a fuller v0.2 fan editor (keyboard nudge, snap-to-grid, invalid-state chart messaging, etc.) after the planning surface is validated on fixtures and hardware; keep `ApplyFanPreset` / `RestoreAutoFan` execution gated until the live write-validation checklist is satisfied.
