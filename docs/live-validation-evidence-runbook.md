...
- If you use **`install-dev-systemd-ratvantage.sh`**, each reinstall **replaces** the
  unit flags; always **`systemctl restart`** after changing them.
- **`install-dev-systemd-ratvantage.sh`** refuses to run if the **`legion-control-daemon`**
  RPM appears installed (`rpm -q legion-control-daemon`); uninstall the RPM first
  or use the packaged unit instead of this dev path.
