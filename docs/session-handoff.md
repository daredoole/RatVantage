...
- Next recommended roadmap slice: optional live-vs-LKG compare hints on the Fans tab, per-profile preset mapping (v0.2), and resume re-apply policy — still without enabling `ApplyFanPreset` / `RestoreAutoFan` in the dashboard until execute-mode validation evidence exists.
