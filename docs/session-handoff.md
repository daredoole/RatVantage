...
- Next recommended task from the updated roadmap: expand manual validation guidance and desktop-session smoke around the existing reversible write set, since the passive refresh/recovery and write-result feedback wiring is now in place for both GTK and tray.
...
1. Add explicit manual validation notes and smoke evidence for the current reversible write set on supported Legion hardware before broadening the live write surface again.
