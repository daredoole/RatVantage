...
1. Continue the automation engine described in [automation-engine-plan.md](automation-engine-plan.md): advanced CPU automation actions are next now that Ryzen backend status exists. Keep CO/advanced CPU automation gated behind explicit warnings and read-back/write-only status. Battery charge type is a real hardware-profile action; same-profile conflicts with conservation mode are rejected; GTK has a starter that saves `fast_charge` / `battery_protect` profiles plus an editable `fast_charge_until_80` rule; automatic execution is opt-in with `--enable-automation-observer`.
