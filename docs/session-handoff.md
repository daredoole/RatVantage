...
- Tray runtime reload now uses a shared client reprobe helper, auto-refreshes after short periodic intervals and suspend-like gaps, suppresses hardware-changing quick actions while refresh state is stale, surfaces recovery/drift notices after reconnect, and emits native desktop notifications when an external platform-profile drift is observed.
