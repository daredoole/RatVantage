...
| Functional Fn-lock | `/sys/bus/platform/devices/VPC2004:*/fn_lock` | `0`, `1`. | Write: yes. Reboot: no. | Probe-only | ideapad_laptop ABI documents it when firmware exposes it. | Changes key behavior; show clear label. | Keyboard page if present |
...
| Camera power | `/sys/bus/platform/devices/VPC2004:*/camera_power` | `0`, `1`. | Write: yes. Reboot: no. | Probe-only | ideapad_laptop ABI says `1` on, `0` off. | Active camera apps may crash or lose device. | Privacy / Peripherals with confirmation |
