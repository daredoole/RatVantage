...
| Touchpad hardware toggle | `/sys/bus/platform/devices/VPC2004:*/touchpad` | `0`, `1`. | Write: yes. Reboot: no. | Probe-only, intentionally blocked | Desktop settings usually handle touchpad better. | Can remove pointer input until re-enabled; keep blocked until fixture capture and recovery validation exist. | Advanced / Peripherals |
...
| Always-on USB charging | `/sys/bus/platform/devices/VPC2004:*/usb_charging` | Usually `0`, `1`. | Write: yes. Reboot: no. | Implemented gated write | Feature charges USB devices while computer is off. | Can drain laptop battery when shut down; require explicit dashboard confirmation and read-back/rollback validation. | Battery / Peripherals |
| Legacy conservation mode | `/sys/bus/platform/devices/VPC2004:*/conservation_mode` | `0`, `1`. | Write: yes. Reboot: no. | Probe-only compatibility, intentionally blocked | Prefer `charge_types` when present. | Avoid double-controlling battery mode through two APIs; keep out of the live write surface while `charge_types` exists. | Hidden compatibility diagnostic |
| Legacy ideapad fan mode | `/sys/bus/platform/devices/VPC2004:*/fan_mode` | `0` silent, `1` standard, `2` dust cleaning, `4` efficient thermal dissipation. | Write: yes. Reboot: no. | Probe-only / intentionally blocked | Hide when Lenovo platform profile/fan curve is available. | Conflicts with modern profile/fan control mental model; keep out of the live write surface while current Legion fan/profile controls exist. | Advanced diagnostics only |
