...
- Add plan-only runtime switch if a kernel/driver path is found. [partially implemented: daemon/D-Bus exposes read-only `PlanGpuModeRuntimeWrite`, but validation blocks until strict GPU mux/session evidence promotes the candidate; there is no executable `SetGpuModeRuntime` daemon path]
...
- Hardware-profile automation with AC/battery/profile/resume/GPU-reboot-completion triggers. [implemented for AC/battery routing as daemon-owned `ac_profile_router` rules, battery capacity threshold routing as daemon-owned `battery_profile_threshold` rules, fast-charge threshold routing as daemon-owned `fast_charge_until_threshold` rules, mapped `resume`, `platform_profile_changed`, `desktop_power_profile_changed`, and `gpu_mode_reboot_completed` hardware-profile triggers through the opt-in observer, GTK custom authoring for AC-router, battery-threshold, and fast-charge rules, and GTK starters for AC routing, AC CPU performance/efficiency routing, battery quiet mode, battery threshold recovery, integrated GPU on battery, resume repair, GPU reboot-completion repair, desktop power repair, and Fn+Q/platform-profile-change tuning repair; broader mixed preset/profile composition remains]
...
- One-click reset plans for every risky tuning family. [partially implemented: `legion-control-ui --reset-diagnostics` emits a read-only reset snapshot for Curve Optimizer reset-to-zero with plan/execute commands, firmware PPT `reset-defaults` with a plan command, OpenRGB SDK keyboard RGB current-mode/color recovery planning with a snapshot-derived plan command, restore-auto-fan and custom-thermal restore-auto-fan plan commands, GPU-mode pending recovery guidance with clear/verify commands, and GPU switching recovery guidance keyed to reboot/session/runtime/unknown switch classifications; compatibility bundles summarize those commands, and GTK Diagnostics surfaces copyable reset diagnostics plus recovery command references; execution remains on the existing gated per-family paths]
