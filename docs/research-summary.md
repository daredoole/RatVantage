...
| Legacy conservation mode | `/sys/bus/platform/devices/VPC2004:*/conservation_mode` | Use only if `charge_types` is absent or as read-only compatibility | Newer Lenovo non-ThinkPad support is better represented by `charge_types`; keep out of the live write surface while `charge_types` exists | Low-medium |
| Legacy ideapad fan mode | `/sys/bus/platform/devices/VPC2004:*/fan_mode` | Hide if Legion platform profile + fan curve are available | Can conflict conceptually with platform profile and custom fan curve; keep out of the live write surface while modern Legion controls exist | Low |
