...
When `--report-dir` is set it also writes desktop/session metadata, `tray-desktop-check.txt`, `tray-menu-check.txt`, tray status/tooltip text, watcher data, raw item properties, and a markdown smoke summary.
...
| 2026-04-25 | KDE Plasma Wayland | `cargo run -p legion-control-tray -- --desktop-check` plus `scripts/smoke-statusnotifier-tray.sh --bus-address "$DBUS_SESSION_BUS_ADDRESS" --hold-seconds 1 --report-dir target/smoke/statusnotifier-kde-wayland-2026-04-25-menu` with fixture daemon on `--session` | Report bundle captured environment, `tray-desktop-check.txt`, `tray-menu-check.txt`, watcher/item properties, tray status, and tooltip text under `target/smoke/statusnotifier-kde-wayland-2026-04-25-menu`. | GNOME-with-extension smoke still required before enabling autostart. |
| 2026-04-25 | KDE Plasma Wayland | Fixture daemon on `--session`, tray on session bus, `busctl` StatusNotifier/DBusMenu checks, screenshot at `target/smoke/statusnotifier-kde-wayland-2026-04-25.png` plus `tray-menu-check.txt` in the report bundle above | Registered item exposed `Id=org.ratvantage.LegionControl`, `Title=Legion Control`, `Category=Hardware`, `Status=Active`, `IconName=applications-system`; menu diagnostics exported current profile/charge rows, battery telemetry, packaged preset labels, capability summary, and Open dashboard/Refresh status/Quit; DBusMenu Refresh succeeded; DBusMenu Quit removed the tray item. | GNOME-with-extension smoke still required before enabling autostart. |
